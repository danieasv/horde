...
type noUpdateRequired struct {
	deviceInfo objects.DeviceInformation
}

func (n *noUpdateRequired) HandleRequest(msg *coap.Message) (*coap.Message, bool, error) {
	switch "/" + msg.PathString() {
	case lwm2m.FirmwareStatePath:
		return reportState(objects.Idle, msg), false, nil

	case lwm2m.DeviceInformationPath:
		return deviceInformationResponse(msg, n.deviceInfo), true, nil
