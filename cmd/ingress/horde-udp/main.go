...
func main() {
	...
	ut := deviceio.NewUDPListener(rxtx.NewRxtxClient(conn), config.UDP)
	if err := ut.Start(); err != nil {
		logging.Error("Unable to start the UDP listener: %v", err)
		os.Exit(3)
	}

	logging.Info("%s started, version=%s (%s)", args[0], version.Number, version.Name)
	utils.WaitForSignal()
	ut.Stop()
}
